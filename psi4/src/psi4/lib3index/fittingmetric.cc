...
FittingMetric::FittingMetric(double eta, std::shared_ptr<BasisSet> aux, bool force_C1)
    : eta_(eta), aux_(aux), is_poisson_(false), is_inverted_(false), force_C1_(force_C1), omega_(0.0) {}
...
void FittingMetric::form_fitting_metric(double eta) {
    eta_ = eta;
    is_inverted_ = false;
    algorithm_ = "NONE";
    ...
    for (int Q = 0; Q < nthread; Q++) {
        if (omega_ > 0.0) {
            ...
            Jint[Q] = std::shared_ptr<TwoBodyAOInt>(rifactory_J.f12g12(exp_coeff));
        }
    }
...
void FittingMetric::form_cholesky_inverse(double eta) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "CHOLESKY";

    form_fitting_metric(eta_);
...
void FittingMetric::form_QR_inverse(double eta, double tol) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "QR";
...
void FittingMetric::form_eig_inverse(double eta, double tol) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "EIG";
...
void FittingMetric::form_full_eig_inverse(double eta, double tol) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "EIG";
...
void FittingMetric::form_full_inverse(double eta) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "FULL";
...
void FittingMetric::form_cholesky_factor(double eta) {
    eta_ = eta;
    is_inverted_ = true;
    algorithm_ = "CHOLESKY";
