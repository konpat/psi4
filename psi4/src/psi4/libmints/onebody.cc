...
void OneBodyAOInt::compute_pair_erfgau(double omega, const libint2::Shell &s1, const libint2::Shell &s2) {
    outfile->Printf(" test compute_pair_erfgau %6.2f \n",omega);
    engine0_->compute(s1, s2);
    for (int chunk = 0; chunk < nchunk(); chunk++) {
        buffers_[chunk] = engine0_->results()[chunk];
    }
...
void OneBodyAOInt::compute_erfgau(double omega, SharedMatrix &result) {
    const auto bs1_equiv_bs2 = (bs1_ == bs2_);
    ...
    for (auto pair : shellpairs_) {
        ...
        outfile->Printf("Before compute_shell_erfgau p1, p2 = %d %d\n", p1, p2);
        compute_shell_erfgau(omega, p1, p2);
        outfile->Printf("After  compute_shell_erfgau p1, p2 = %d %d\n", p1, p2);
        const double *location = buffers_[0];
        for (int p = 0; p < ni; ++p) {
            for (int q = 0; q < nj; ++q) {
                result->add(0, i_offset + p, j_offset + q, *location);
...
void OneBodyAOInt::compute_erfgau(double omega, std::vector<SharedMatrix> &result) {
    // Do not worry about zeroing out result
    int ns1 = bs1_->nshell();
    int ns2 = bs2_->nshell();
    const auto bs1_equiv_bs2 = (bs1_ == bs2_);
    ...
    for (const auto &pair : shellpairs_) {
        ...
        int i_offset = bs1_->shell_to_basis_function(p1);
        int j_offset = bs2_->shell_to_basis_function(p2);
        outfile->Printf("compute_pair_erfgau 1\n");
        // Compute the shell
        compute_pair_erfgau(omega, s1, s2);
        outfile->Printf("compute_pair_erfgau s1, s2 = %d %d\n", s1, s2);
        // For each integral that we got put in its contribution
        for (int r = 0; r < nchunk_; ++r) {
            const double *location = buffers_[r];
            for (int p = 0; p < ni; ++p) {
...
void OneBodyAOInt::compute_shell_erfgau(double omega, int sh1, int sh2) {
    const auto &s1 = bs1_->l2_shell(sh1);
    const auto &s2 = bs2_->l2_shell(sh2);
    outfile->Printf("test compute shell erfgau %6.2f %d %d\n",omega,s1,s2);
    compute_pair_erfgau(omega, s1, s2);
}
