...
DiskDFJK::DiskDFJK(double eta, std::shared_ptr<BasisSet> primary, std::shared_ptr<BasisSet> auxiliary, Options& options)
    : JK(primary), auxiliary_(auxiliary), options_(options) {
    eta_ = eta;
    common_init(eta_);
}
...
void DiskDFJK::common_init(double eta) {
    eta_ = eta;
    df_ints_num_threads_ = 1;
#ifdef _OPENMP
    df_ints_num_threads_ = Process::environment.get_n_threads();
...
void DiskDFJK::initialize_temps(double eta) {
    eta_ = eta;
    J_temp_ = std::make_shared<Vector>("Jtemp", n_function_pairs_);
    D_temp_ = std::make_shared<Vector>("Dtemp", n_function_pairs_);

    J_temp_sr_ = std::make_shared<Vector>("Jtemp_sr", n_function_pairs_sr_);
    D_temp_sr_ = std::make_shared<Vector>("Dtemp_sr", n_function_pairs_sr_);
...
void DiskDFJK::free_temps(double eta) {
    eta_ = eta;
    J_temp_.reset();
    D_temp_.reset();
...
void DiskDFJK::preiterations(double eta) {
    // Setup integral objects
    eta_ = eta;
    eri_.clear();
    std::shared_ptr<BasisSet> zero = BasisSet::zero_ao_basis_set();
...
void DiskDFJK::compute_JK(double eta) {
    eta_ = eta;
    // zero out J, K, and wK matrices
    zero();
...
void DiskDFJK::initialize_JK_core(double eta) {
    eta_ = eta;
    size_t three_memory = ((size_t)auxiliary_->nbf()) * n_function_pairs_;
    size_t two_memory = ((size_t)auxiliary_->nbf()) * auxiliary_->nbf();
    ...
    const std::vector<long int>& schwarz_shell_pairs_sr = f12g12_.front()->shell_pairs_to_dense();
    const std::vector<long int>& schwarz_fun_pairs_sr = f12g12_.front()->function_pairs_to_dense();
...
void DiskDFJK::initialize_JK_disk(double eta) {
    eta_ = eta;
    // Try to load
    if (df_ints_io_ == "LOAD") {
        return;
...
#pragma omp parallel for schedule(guided) num_threads(nthread)
        for (int MUNU = MN_start_val; MUNU < MN_start_val + MN_col_val; MUNU++) {
            ...
            for (int P = 0; P < auxiliary_->nshell(); P++) {
                ...
                int p = auxiliary_->shell(P).function_index();
                eri_[rank]->compute_shell(P, 0, MU, NU);
                f12g12_[rank]->compute_shell(P, 0, MU, NU);
                const auto *buffer = buffers[0];
                const auto *buffer_sr = buffers_sr[0];
...
void DiskDFJK::manage_JK_core(double eta) {
    eta_ = eta;
    for (int Q = 0; Q < auxiliary_->nbf(); Q += max_rows_) {
        int naux = (auxiliary_->nbf() - Q <= max_rows_ ? auxiliary_->nbf() - Q : max_rows_);
        if (do_J_) {
...
void DiskDFJK::manage_JK_disk(double eta) {
    eta_ = eta;
    Qmn_ = std::make_shared<Matrix>("(Q|mn) Block", max_rows_, n_function_pairs_);
    psio_->open(unit_, PSIO_OPEN_OLD);
    for (int Q = 0; Q < auxiliary_->nbf(); Q += max_rows_) {
        ...
        if (do_J_) {
            timer_on("JK: J");
            block_J(eta_, &Qmn_->pointer()[0], naux);
            timer_off("JK: J");
        }
        if (do_K_) {
            timer_on("JK: K");
...
void DiskDFJK::block_J(double eta, double** Qmnp, int naux) {
    const std::vector<std::pair<int, int> >& function_pairs = eri_.front()->function_pairs();
    ...
    for (size_t N = 0; N < J_ao_.size(); N++) {
        ...
        double** Jsrp = J_ao_[N]->pointer();

        double* J2p = J_temp_->pointer();
        double* D2p = D_temp_->pointer();
   
        double* J2srp = J_temp_sr_->pointer();
        double* D2srp = D_temp_sr_->pointer();
        ...
        for (size_t mn = 0; mn < num_nm; ++mn) {
            ...
            int i = function_pairs_sr[mn].first;
            int j = function_pairs_sr[mn].second;
...
void DiskDFJK::block_K(double eta, double** Qmnp, int naux) {
    eta_ = eta;
    const std::vector<std::pair<int, int> >& function_pairs = eri_.front()->function_pairs();
    const std::vector<long int>& function_pairs_to_dense = eri_.front()->function_pairs_to_dense();

    const std::vector<std::pair<int, int> >& function_pairs_sr = f12g12_.front()->function_pairs();
    const std::vector<long int>& function_pairs_to_dense_sr = f12g12_.front()->function_pairs_to_dense();
    ...
    for (size_t N = 0; N < K_ao_.size(); N++) {
        ...
        double** Clsrp = C_left_ao_[N]->pointer();

        double** Crp = C_right_ao_[N]->pointer();

        double** Crsrp = C_right_ao_[N]->pointer();
...
#pragma omp parallel for schedule(dynamic)
            for (int m = 0; m < nbf; m++) {
                ...
                const std::vector<int>& pairs = eri_.front()->significant_partners_per_function()[m];
                const std::vector<int>& pairs_sr = f12g12_.front()->significant_partners_per_function()[m];
